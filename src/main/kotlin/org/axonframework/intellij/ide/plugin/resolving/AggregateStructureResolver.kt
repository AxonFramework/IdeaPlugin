...
class AggregateStructureResolver(private val project: Project) {
    ...
    private fun resolve(): List<Entity> {
        val annotationResolver = project.annotationResolver()
        // Search for both v4 (@AggregateRoot) and v5 (@EventSourcedEntity) annotations
        val aggregateAnnotations = listOf(
            AxonAnnotation.AGGREGATE_ROOT,
            AxonAnnotation.EVENT_SOURCED_ENTITY
        )

        return aggregateAnnotations
            .flatMap { annotationResolver.getAnnotationClasses(it) }
            .flatMap { AnnotatedElementsSearch.searchPsiClasses(it.psiClass, project.axonScope()).findAll() }
            .distinct()
